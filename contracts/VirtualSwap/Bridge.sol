 ...
 *
 * 1) tokenToSynth
 * Swaps a supported token in a invader pool to any synthetic asset (e.g. tBTC -> sAAVE).
 *
 * 2) synthToToken
 * Swaps any synthetic asset to a suported token in a Invader pool (e.g. sDEFI -> USDC).
 *
 * 3) tokenToToken
 * Swaps a supported token in a Invader pool to one in another pool (e.g. renBTC -> DAI).
 *
...
contract Bridge is ERC721 {
     ...
     */
    constructor() public ERC721("Invader Cross-Asset Swap", "InvaderSynthSwap") {
    ...
    /**
     * @notice Settles and withdraws the synthetic asset without swapping it to a token in a Invader pool. Only the owner
     ...
     * @param swap the address of a Invader pool to use to swap the given token to a bridging synth
     ...
     * @param swaps the addresses of the two Invader pools used to do the cross-asset swap
    ...
    /**
     * @notice Initiates a cross-asset swap from a token in one Invader pool to one in another. The caller will receive
     * an ERC721 token representing their ownership of the pending cross-asset swap.
     * @param swaps the addresses of the two Invader pools used to do the cross-asset swap
     * @param tokenFromIndex the index of the token in the first `swaps` pool to swap from
     * @param tokenToIndex the index of the token in the second `swaps` pool to swap to
